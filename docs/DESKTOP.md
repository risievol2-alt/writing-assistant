...
工作流会验证标签、运行全部测试，并将 NSIS 安装包上传到同名 GitHub Release。

### Windows 代码签名

Release 工作流要求仓库配置以下 GitHub Actions Secrets：
...
任一 Secret 缺失、PFX 无私钥、证书过期或签名验证失败都会终止发布。Base64 只是一种文本编码，真正的保护来自 GitHub Actions Secrets；不要把 `.pfx`、私钥或 Base64 文件提交到 Git。
